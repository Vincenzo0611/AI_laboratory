...
def zad1_single_neuron(student_id):
    ...
    class SingleNeuron:
        ...
        def forward(self, x_data):
            ...
            y = self.f_act(x_data.dot(self.W) + self.b)
            return y
...
def zad2_two_layer_net(student_id):
    ...
    model.hidden_layer.W[:, 0] = [-1, 0]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [1, 0]      # wagi neuronu h2
    model.hidden_layer.b[:] = [0, -0.9]         # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [1, 1]      # wagi neuronu wyjściowego
    model.output_layer.b[:] = -0.3         # bias neuronu wyjściowego
...
if __name__ == '__main__':
    ...
    student_id = 193218         # Twój numer indeksu, np. 102247
